...
<body class="min-h-screen flex items-center justify-center">
    <div class="cyberpunk-bg p-8 rounded-lg max-w-md w-full text-center">
        <h1 class="text-4xl font-bold neon-text mb-4">ITSBAD PRO</h1>
        <p class="text-green-400 mb-4">SYSTEM ONLINE</p>
        <div class="mb-4">
            <p class="text-white mb-2">Your UUID:</p>
            <code class="bg-gray-800 p-2 rounded text-green-400 block">${uuid}</code>
            <button onclick="navigator.clipboard.writeText('${uuid}')" class="mt-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">Copy UUID</button>
        </div>
        <div class="space-y-2">
            <button onclick="window.location.href='/api/v1/sub?format=raw'" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded w-full">Get VLESS Config</button>
            <button onclick="window.location.href='/api/v1/sub?format=clash'" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded w-full">Get Clash Config</button>
        </div>
    </div>
</body>
</html>
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === '/' || url.pathname === '') {
        const uuid = userID || crypto.randomUUID();
        return new Response(getItsbadUI(APP_DOMAIN, uuid), {
          headers: {
      ...
      if (upgradeHeader === "websocket") {
        ...
        if (url.pathname.length == 3 || url.pathname.match(",")) {
          ...
          proxyIP = kvPrx[prxKey][Math.floor(Math.random() * kvPrx[prxKey].length)];
...
async function checkPrxHealth(proxyIP, prxPort) {
  const req = await fetch(`${PRX_HEALTH_CHECK_API}?ip=${proxyIP}:${prxPort}`);
